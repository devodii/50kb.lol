...
export class FileUploadApi {
  ...
  async upload(
    ...
    const utFiles = filesWithMetadata.map(
      (file) =>
        new UTFile([file], file.name, {
          customId: JSON.stringify(file.metadata),
        }),
    );
  ...
  async delete(fileKeys: string[]) {
    ...
    if (!response.success) {
      throw new Error(
        `Failed to delete files: ${response.deletedCount} files not deleted`,
      );
    }
  ...
  async replace(
    oldFileKeys: string[],
    newFilesWithMetadata: FileWithMetadata[],
  ) {
